...
pca <- function(final_pca_data){
  ...
  p <- ggplot(final_pca_data , aes(x = .fittedPC1, y = .fittedPC2)) +
    ...
    scale_color_gradientn(colours = c("darkgreen", "green4", "green3", "green2","olivedrab1", "yellow", "white"),limits = c(0,400)) +
    labs(title = "Fig3: PCA to depict similar shooting trends among states", x = "PC1", y = "PC2") +
    theme_bw() +
...
area <- function(data, selected){
  ...
  area_data %>%
    ...
    labs(title = paste0("Fig2: Breakdown of Total shooting cases into deaths & injuries per month\n"), x = "Month",y = "Total Cases") +
...
map <- function(data, selected){
  ...
  map_data %>%
    ...
    scale_fill_gradientn(colours = c("maroon", "palevioletred1", "pink1", "pink", "white"),limits = c(0,400),guide = guide_colorbar(title.position = "top")) +

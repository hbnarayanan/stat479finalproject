                      ...
                      tabItems(
                        ...
                        tabItem("one",
                                fluidPage(
                                  ...
                                  fluidRow(column(6, plotlyOutput("Chi"), height = 500), 
                                           column(6,selectInput("cats", "Choose Categorical Variable", categorical),plotlyOutput("bar_plot_cat"))
                                )
                        )
                        ),
...
server <- function(input, output) {
  ...
  output$density <- renderPlot({
    ...
    if(length(s)){
      row <- cat_num()[s, ]
      ggplot(job_Ayushi, aes(x = .data[[row$Numerical]], y = as.factor(.data[[row$Categorical]]), fill = Attrition)) +
        geom_density_ridges() +
